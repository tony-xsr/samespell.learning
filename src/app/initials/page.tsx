...
import { getInitialLanguages, wordCount } from "@/lib/vocabStore";

const LANG_ICON: Record<string, string> = { zh: "🇨🇳", ko: "🇰🇷", ja: "🇯🇵" };

export default async function InitialsHome() {
  const languages = await getInitialLanguages();
    ...
    <main className="flex flex-1 flex-col items-center px-4 py-10 sm:py-16">
      <div className="w-full max-w-md">
        ...
        <div className="mt-3 rounded-3xl bg-gradient-to-br from-accent-500 to-brand-600 p-6 text-white shadow-lg">
          <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">Cùng âm đầu pinyin</h1>
          <p className="mt-2 text-sm text-white/90">
            Các từ chỉ có điểm chung là cùng PHỤ ÂM ĐẦU trong pinyin (vd tất cả bắt đầu bằng &quot;x&quot;)
            — âm vần, thanh điệu và nghĩa hoàn toàn khác nhau. Dễ lẫn khi gõ hoặc nghe âm đầu lúc mới
            học.
          </p>
        </div>
        ...
        <div className="mt-6 flex flex-col gap-3">
          {languages.map((lang) => {
            ...
            return (
              ...
              >
                <Link
                  href={disabled ? "#" : `/initials/${lang.language}`}
                    ...
                    <div className="mt-0.5 text-xs text-ink-muted">
                      {lang.groups.length > 0 ? (
                        <span className="inline-flex items-center gap-1 rounded-full bg-brand-50 px-2 py-0.5 font-medium text-brand-700">
                          {lang.groups.length} nhóm âm đầu · {total} từ
                        </span>
                      ) : (
                        "Sắp ra mắt"
                      )}
