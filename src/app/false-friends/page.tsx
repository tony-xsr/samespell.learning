...
import { getFalseFriendLanguages, wordCount } from "@/lib/vocabStore";

const LANG_ICON: Record<string, string> = { zh: "🇨🇳", ko: "🇰🇷", ja: "🇯🇵" };

export default async function FalseFriendsHome() {
  const languages = await getFalseFriendLanguages();
    ...
    <main className="flex flex-1 flex-col items-center px-4 py-10 sm:py-16">
      <div className="w-full max-w-md">
        ...
        <div className="mt-3 rounded-3xl bg-gradient-to-br from-accent-500 to-brand-600 p-6 text-white shadow-lg">
          <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">Bẫy nghĩa</h1>
          <p className="mt-2 text-sm text-white/90">
            Một chữ Hán DÙNG CHUNG xuất hiện trong hai (hay nhiều) từ ghép, nhưng nghĩa của các từ ghép
            đó lệch hẳn hoặc trôi đi so với nghĩa gốc của chữ — dễ đoán sai nghĩa nếu chỉ nhớ máy móc
            nghĩa của từng chữ riêng lẻ.
          </p>
        </div>
        ...
        <div className="mt-6 flex flex-col gap-3">
          {languages.map((lang) => {
            ...
            return (
              ...
              >
                <Link
                  href={disabled ? "#" : `/false-friends/${lang.language}`}
                    ...
                    <div className="mt-0.5 text-xs text-ink-muted">
                      {lang.groups.length > 0 ? (
                        <span className="inline-flex items-center gap-1 rounded-full bg-brand-50 px-2 py-0.5 font-medium text-brand-700">
                          {lang.groups.length} nhóm bẫy nghĩa · {total} từ
                        </span>
                      ) : (
                        "Sắp ra mắt"
                      )}
