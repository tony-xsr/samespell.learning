...
export default function ChainBrowser({ language, data }: { language: Language; data: ChainLanguageData }) {
  ...
  const onChains = !hasClusters || activeTab === "chains";

  // Accessor đổi theo tab đang xem; trong lúc mở toàn màn hình thì không đổi tab được nên ổn định.
  const accessors = useMemo<ClusterBrowserAccessors>(() => {
    if (onChains) {
      return {
        count: chains.length,
        idAt: (i) => chains[i]?.id,
        wordsAt: (i) => chains[i]?.nodes.map((n) => ({ headword: n.headword, meaningVn: n.meaningVn })) ?? [],
      };
    }
    return {
  ...
  const openChain = onChains && ui.openIndex !== null ? chains[ui.openIndex] : undefined;
  const openCluster = !onChains && ui.openIndex !== null ? clusters[ui.openIndex] : undefined;
        ...
        <div className={listClass}>
          {clusters.map((cluster, i) => (
            <div key={cluster.id} className="relative h-full">
              ...
              <ClusterCard cluster={cluster} layout={viewMode} onOpen={() => ui.open(i)} />
            </div>
          ))}
        </div>
      )}

      <ClusterBrowserShell ui={ui} headerBadge={openChain ? `${openChain.nodes.length} mắt xích` : undefined}>
        {openChain && (
          <ChainDetailPanel
            key={openChain.id}
            chain={openChain}
